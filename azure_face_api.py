...
class personGroup:
  ...
  def __init__(self, groupId = "csclub"):
    ...
    save_json_data = {}
    save_data_path = os.path.join(*[self.CURR_DIR, "logs","data.txt"])
    infile = open(save_data_path, "r")
    if(len(infile.read()) != 0):
      infile.seek(0)
      save_json_data = json.load(infile)
      self.personGroupId = groupId#list(save_json_data.keys())[0]
  ...
  def add_person(self, personName = "", studentId = "", studentEmail = ""):
    if(studentId in self.studentIds.keys()):
      print("StudentId [{}] already exists".format(studentId))
      return self.studentIds[studentId]
  ...
  def add_faces(self, personId = "", face_urls = []):
    ...
    try:
      ...
      print("could not add face urls to {} :{}".format(personId, e))
  ...
  def delete_person_group(self):
    ...
    try:
      ...
      #request
      self.makeRequest(method = "DELETE", endpoint = "/face/v1.0/persongroups/{personGroupId}?%s" % params, body = "{body}")
